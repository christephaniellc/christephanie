...
const WeddingAttendanceRadios = ({ guestId }: { guestId: string }) => {
  ...
  const response = useMemo(() => {
    ...
    let setAllergyResponse = guest.preferences.foodAllergies !== null 
      && guest.preferences.foodAllergies.join('') !== '' && guest.preferences.foodAllergies.join('') !== 'none';
  ...
  return (
    <Box display={'flex'} alignItems="center" width="100%" sx={{
      fontSize: '0.6rem', overflowY: 'auto' }}>
      {declined && (
        <Typography mr={2}>
          <StickFigureIcon
            fontSize="small"
