...
function FoodAllergies({ guestId }: { guestId: string }) {
  ...
  return (
    <Box display="flex" flexWrap="wrap" justifyContent="space-between"
         border={`1px dashed ${theme.palette.secondary.main}`} p={2} overflow={'auto'} height="100%"
          minWidth={400}
    >
      <Chip
        sx={{ backdropFilter: 'blur(20px)', backgroundColor: 'rgba(0,0,0,.6)', my: 1, width: '100%' }}
      ...
      {allergyIconProps.map((allergy) => {
        ...
        return <Chip
          sx={{
            backdropFilter: 'blur(20px)', backgroundColor: 'rgba(0,0,0,.6)',
            my: 0.5,
            cursor: guest.preferences.foodAllergies.includes(allergy.allergyName) ? 'auto' : 'pointer',
          }}
          variant={guest.preferences.foodAllergies.includes(allergy.allergyName) ? 'outlined' : 'filled' as 'outlined' | 'filled'}
          color={guest.preferences.foodAllergies.includes(allergy.allergyName) ? 'error' : 'primary' as 'primary' | 'error'}
          icon={<allergy.icon />}
          disabled={familyActions.patchFamilyMutation.status === 'pending' || familyActions.getFamilyUnitQuery.isFetching}
